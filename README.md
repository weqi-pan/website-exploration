...
skills.sh 的自动审计（Snyk / Socket）对本技能给出 **Warn（建议复审，非失败）**，两条意见与既有缓解措施：

| 审计意见 | 缓解 |
|---|---|
| 浏览器自动化会接触第三方页面内容，存在间接提示注入风险（Snyk W011，风险分 0.10） | 页面文本只作为数据读取；同域范围限定；危险操作拦截；证据脱敏。该风险是浏览器自动化技能的固有属性，无法完全消除 |
| RPC 服务缺少鉴权，若绑定到本机以外会有风险（Socket） | 代码层面强制绑定 `127.0.0.1`/`localhost`/`::1`，其它 `--host` 直接拒绝启动 |
